  ...
  delete = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
      ...
      if (!childBehavior || !childBehavior.childId || !childBehavior.behaviorId) {
        return res
          .status(400)
          .json({ error: "data (childId, behaviorId) is required" })
      }
      const id = { childId: childBehavior.childId, behaviorId: childBehavior.behaviorId }
      const deletedChildBehavior = await this.childBehaviorModel.deleteChildBehavior(id)
      res.status(200).json({ childBehavior: deletedChildBehavior })
    } catch (error) {
      res.status(500).json({ error: "Error deleting childBehavior" })
      next(error)
    }
  }
}
